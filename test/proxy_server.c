...
int main(int argc, char **argv) {
	...
	if ((lcfd = socket(AF_INET, SOCK_STREAM, 0)) == -1 || (lgfd = socket(
	...
	if (bind(lcfd, (struct sockaddr *) (&scin), sizeof(struct sockaddr)) == -1
			|| bind(lgfd, (struct sockaddr *) (&sgin), sizeof(struct sockaddr))
					== -1) {
...
int net_tcp_recv(int fd) {
	if (fd == lcfd) {
		...
		extdata->isuse = 0;
		tcp_conn_t *tconn = new_tcpconn(cfd, GWLINK_START, cin.sin_port,
				"0.0.0.0", client_port, extdata);
		if (tconn == NULL) {
			free(extdata);
			return 1;
		}
		...
		extdata->isuse = 0;
#ifdef GWLINK_WITH_SOCKS5_PASS
		tcp_conn_t *tconn = new_tcpconn(gfd, GWLINK_AUTH, cin.sin_port,
				"0.0.0.0", gateway_port, extdata);
#else
		tcp_conn_t *tconn =
			...
			if (extdata->way == CONN_WITH_CLIENT) {
				if (!extdata->toconn) {
					...
					if (p_list->p_head != NULL) {
						ext_conn_t *todata;
						for (t_list = p_list->p_head; t_list != NULL; t_list
								= t_list->next) {
							todata = t_list->extdata;
							if (todata->way == CONN_WITH_SERVER
									&& !todata->isuse && t_list->gwlink_status
									== GWLINK_START) {
			...
			if (extdata->way == CONN_WITH_SERVER) {
				...
				case GWLINK_PASS:
					...
					if (dlen > 0 && dbuf[0] == 0x1) {
						if (dlen > 1 && dbuf[1] + 3 < dlen && 3 + dbuf[1]
								+ dbuf[2 + dbuf[1]] <= dlen) {
							char user[128] = { 0 };
							char pass[128] = { 0 };
							memcpy(user, dbuf + 2, dbuf[1]);
							memcpy(pass, dbuf + 3 + dbuf[1], dbuf[2 + dbuf[1]]);
							if (!strcmp(user, get_auth_user()) && strlen(user)
									== strlen(get_auth_user()) && !strcmp(pass,
									get_auth_pass()) && strlen(pass) == strlen(
									get_auth_pass())) {
								char rebuf[2] = { 0x1, 0 };
								if (send(fd, rebuf, 2, 0) > 0) {
